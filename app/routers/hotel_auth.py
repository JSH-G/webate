...
router = APIRouter(tags=['Authentication_Hotel'])


@router.post('/login_hotel',status_code=status.HTTP_200_OK)
def login_hotel(device_token: str = Body(None), user_credentials: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):

    user = db.query(models.Hotel_Sign_up).filter(models.Hotel_Sign_up.email == user_credentials.username.lower())
    ...
    if not utils.verify(user_credentials.password, up_pass.password):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not efound Credential")
